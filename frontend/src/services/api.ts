...
export const worldsApi = {
  ...
  generate: (request: { keywords: string; generate_characters?: boolean; character_count?: number }): Promise<{
    world: World;
  ...
  generateStream: (
    request: { keywords: string; generate_characters?: boolean; character_count?: number },
    onProgress: (message: string, progress: number) => void,
    onComplete: (result: any) => void,
    onError: (error: string) => void
  ) => {
    const eventSource = new EventSource(`${API_BASE_URL}/worlds/generate-stream?keywords=${encodeURIComponent(request.keywords)}&generate_characters=${request.generate_characters}&character_count=${request.character_count}`);
